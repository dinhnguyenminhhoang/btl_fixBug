...
void themGioHang(int x,int y,node *&head,node *&GioHang,int sobang){//can lay gia tri sobang de xoa bang cu
...
//	int i=1;// vi gia tri rac nen lay tu 1
//	while(tempGioHang!=NULL){//gia tri dau dang nhan gia tri rac nen co the in truc tiep node tiep theo 
//		tempGioHang=tempGioHang->next;
//		i++;
//		tempGioHang->display();
...
int i=2;
	...
	while(tempGioHang!=NULL){
			...
			while(TempHead->next!=NULL){//add data at the end of the list
				TempHead=TempHead->next;
			}
...
void TimSanPham::LocTheoGia(int x,int y,node *head){
	...
	displaybox(x+20,y,3,15);
		do{
		gotoxy(x+23,y+1);
		for(int i=0;i<GiaCanTim.length();i++){
			cout<<" ";
		}
		gotoxy(x+23,y+1);
		getline(cin,GiaCanTim);
	}while(isNumber(GiaCanTim)==false);
	int gia=stoi(GiaCanTim);
	
	int dem=0;
	node *temp= new node;
	temp =head;
	while(temp!=NULL){//lay gia tri dem de in bang
		if(temp->getgiaban()<=gia){
			dem++;
		}
		temp=temp->next;
	}
...
void SaveFileGioHang(node *a){
	...
	while(head!=NULL){
		...
		FileOut << endl << "- Ten san pham :" <<head->gettensp();
		FileOut << endl << "- Xuat xu :" << head->getxuatxu();
		FileOut << endl << "- Dung tich :" << head->getdungtich();
		FileOut << endl << "- Nong do :" << head->getnongdo();
		FileOut << endl << "- Gia ban :" << head->getgiaban();
...
void Timten(node *&head,string tensp,node *&GioHang){//GioHang dang nhan gia tri rac khi chay vi khong khoi tao gio hang = NULL duoc nen phai xoa node dau di
	node *temp=new node ;
	node *tempGioHang= new node;
	tempGioHang = GioHang;
	int i=1;
	...
	while(kt){
		...
		switch(c){
			...
			case 13:
				string tensp;
				string ma;
				if(y==ycucu){//chon theo ten san pham
				setColor(3,4);
	...
	for(int i= y;i<height+y;i++){
		...
		if(j==x||j==x+width-1){
			...
			cout<<char(179);
			 ...
			 if(j==x+width-1 &&i==y){//goc tren ben phai
			 gotoxy(j,i);
				cout<<char(191);
			}
			 if(j==x+width-1 && i==y+height-1){//goc tren ben trai 
			 gotoxy(j,i);
				cout<<char(217);
			}
			if(i==y && j==x){//goc tren ben trai
				gotoxy(j,i);
				cout<<char(218);
				
			}
			if(i==y+height-1 && j==x){//goc tren ben phai 
				gotoxy(j,i);
				cout<<char(192);
			}
		}
...
//					InBangDanhSachDaSapXep(3,y+5,head,sobang);
				...
				if(x==50){//chon san pham
					for(int i=5;i<11;i++){//in de xoa bang cu 
					for(int j=0;j<110;j++){
						SetBGColor(0);
						gotoxy(j,i);
						cout<<" ";
...
//                        index=temp;
 ...
 void DocDuLieuTuFile(int x,int y,node *&head,int &sobang){//lay du lieu ra. Nay la ham friend cua class node
	ifstream inFile;
	sobang=0;
	head= new node;
	node *tail= new node;
...
void MenuCuaHam(int x,int y){//=====================================================HAM CHINH============================================================
...
void InBangDanhSachDaSapXep(int x,int y,node *head,int sobang){
	for(int i=x;i<110+x;i++){
	  ...
	  SetBGColor(1);
	textcolor(10);
	gotoxy(8,y+1); cout<<"STT";
	gotoxy(17,y+1); cout<<"MA";
	gotoxy(34,y+1); cout<<"TEN SAN PHAM";
	gotoxy(59,y+1); cout<<"XUAT XU";
	gotoxy(72,y+1); cout<<"DUNG TICH(ml)";
	gotoxy(88,y+1); cout<<"NONG DO";
	gotoxy(103,y+1);cout<<"GIA BAN";
	}
//	cout<<sobang;
